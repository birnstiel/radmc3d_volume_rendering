...
double precision :: transfer_density_mean
double precision :: transfer_density_sigm
...
subroutine userdef_defaults()
  ...
  ! The default values
  !
  transfer_density_mean = 1d0
  transfer_density_sigm = 10d0
  !
...
subroutine userdef_action()
  ...
  if(fex) then
     ...
     close(1)
     call parse_input_double('transfer_density_mean@        ',transfer_density_mean)
     call parse_input_double('transfer_density_sigm@        ',transfer_density_sigm)
  endif
...
subroutine userdef_srcalp(index,nrfreq,inu0,inu1,freq,src,alp)
  ...
  if(index.gt.0) then
     ...
     ! transfer functions.
     !
     src(:) = exp(-(gasdens(index) - transfer_density_mean)**2 / (2d0 * transfer_density_sigm**2))
     !
